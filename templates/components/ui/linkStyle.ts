...
import { type BtnLinkStyleArgsType } from "@/types/ui";

export const btnLinkStlye = tv({
  base: "text-white text-center inline-block outline-none",
  variants: {
    color: {
...
export function generateBtnLinkStyle({
  color,
  size,
  full,
  className,
}: BtnLinkStyleArgsType) {
  const base = btnLinkStlye({ color, size, full: full ? "full" : "base" });
